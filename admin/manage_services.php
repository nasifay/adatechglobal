<?php
// admin/manage_services.php - CRUD for services
session_start();
    ...
    try {
        if ($_POST['form'] === 'add') {
            ...
            if (!empty($_POST['existing_image'])) {
                ...
                $uploaded = handle_image_upload('image', __DIR__ . '/../assets/img/services');
                $imageName = basename($uploaded);
            }
            $stmt = $pdo->prepare('INSERT INTO services (title, summary, description, image) VALUES (:title, :summary, :description, :image)');
            $stmt->execute([':title'=>$title,':summary'=>$summary,':description'=>$description,':image'=>$imageName]);
            header('Location: manage_services.php?msg=added'); exit;
        }
        ...
        if ($_POST['form'] === 'edit') {
            ...
            if ($imageName) {
                $stmt = $pdo->prepare('UPDATE services SET title=:title, summary=:summary, description=:description, image=:image WHERE id=:id');
                $stmt->execute([':title'=>$title,':summary'=>$summary,':description'=>$description,':image'=>$imageName,':id'=>$id]);
            } else {
                $stmt = $pdo->prepare('UPDATE services SET title=:title, summary=:summary, description=:description WHERE id=:id');
                $stmt->execute([':title'=>$title,':summary'=>$summary,':description'=>$description,':id'=>$id]);
            }
            header('Location: manage_services.php?msg=updated'); exit;
        }
        ...
        if ($_POST['form'] === 'delete') {
            ...
            if ($row && !empty($row['image'])) {
                $file = __DIR__ . '/../assets/img/services/' . $row['image'];
                if (is_file($file)) @unlink($file);
            }
            $stmt = $pdo->prepare('DELETE FROM services WHERE id = :id');
            $stmt->execute([':id'=>$id]);
            header('Location: manage_services.php?msg=deleted'); exit;
        }
...
$imgStmt = $pdo->prepare("SELECT * FROM images WHERE type IN ('services','uploads','other') ORDER BY uploaded_at DESC");
$imgStmt->execute();
...
<?php require_once __DIR__ . '/partials/sidebar.php'; ?>
    <div class="admin-content">
        ...
        <form action="manage_services.php" method="post" enctype="multipart/form-data">
            ...
            <div class="admin-form-row"><label>Title<br><input type="text" name="title" required></label></div>
            <div class="admin-form-row"><label>Summary<br><input type="text" name="summary"></label></div>
            <div class="admin-form-row"><label>Description<br><textarea name="description" rows="4"></textarea></label></div>
            <div class="admin-form-row"><label>Image<br><input type="file" name="image" accept="image/*"></label></div>
            <div class="admin-form-row"><label>Or choose existing image<br>
                <select name="existing_image"><option value="">-- none --</option>
                    <?php foreach ($imgs as $img): ?><option value="<?php echo esc($img['filename']); ?>"><?php echo esc($img['type'].'/'.$img['filename']); ?></option><?php endforeach; ?>
                </select>
            </label></div>
        ...
        <table>
            <thead><tr><th>ID</th><th>Title</th><th>Summary</th><th>Image</th><th>Actions</th></tr></thead>
            <tbody>
            <?php foreach ($services as $s): ?>
                <tr>
                    <td><?php echo (int)$s['id']; ?></td>
                    <td><?php echo esc($s['title']); ?></td>
                    <td><?php echo esc(mb_strimwidth($s['summary'],0,80,'...')); ?></td>
                    <td><?php if ($s['image']): ?><img src="<?php echo esc(asset('assets/img/services/' . $s['image'])); ?>" style="height:40px"><?php endif; ?></td>
                    <td>
        ...
        <?php if ($action === 'edit' && !empty($service)): ?>
            ...
            <form action="manage_services.php" method="post" enctype="multipart/form-data">
                ...
                <div class="admin-form-row"><label>Title<br><input type="text" name="title" value="<?php echo esc($service['title']); ?>" required></label></div>
                <div class="admin-form-row"><label>Summary<br><input type="text" name="summary" value="<?php echo esc($service['summary']); ?>"></label></div>
                <div class="admin-form-row"><label>Description<br><textarea name="description" rows="4"><?php echo esc($service['description']); ?></textarea></label></div>
                <div class="admin-form-row">Current Image: <?php if ($service['image']): ?><img src="<?php echo esc(asset('assets/img/services/' . $service['image'])); ?>" style="height:40px"><?php else: ?>None<?php endif; ?></div>
                <div class="admin-form-row"><label>Replace Image<br><input type="file" name="image" accept="image/*"></label></div>
                <div class="admin-form-row"><label>Or choose existing image<br>
                    <select name="existing_image"><option value="">-- none --</option>
                        <?php foreach ($imgs as $img): ?><option value="<?php echo esc($img['filename']); ?>"><?php echo esc($img['type'].'/'.$img['filename']); ?></option><?php endforeach; ?>
                    </select>
