-- All-in-one migration and seed for Adatech CMS (hosting-safe)
-- This version removes the CREATE DATABASE/USE statements so it can be imported
-- by a hosting user who does not have permission to create databases.
-- Replace `your_database_name` below with the database name provided by your host,
-- or select the target database in phpMyAdmin before importing.
...
-- Note: some hosts (older MySQL) do not support CREATE INDEX IF NOT EXISTS —
-- phpMyAdmin/import will ignore errors or you can remove the next line if import fails.
